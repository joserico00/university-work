...
|---|---|---|---|
| [pandas_series_and_dataframes](pandas_series_and_dataframes.ipynb) | Series and DataFrame basics | Indexing, `RangeIndex`, `%timeit` (Python `sum` vs `np.sum`), dict to DataFrame, `savetxt`/`loadtxt` vs `read_csv`, adding columns/rows, sorting | pandas, numpy |
| [pandas_statistics_and_filtering](pandas_statistics_and_filtering.ipynb) | Math, statistics and filtering | `mean`/`sum` by axis, `idxmin`, boolean masks, `loc` with conditions | pandas |
| [data_cleaning_missing_values_duplicates](data_cleaning_missing_values_duplicates.ipynb) | Missing data, duplicates, replacement | `isna`, `dropna(how, thresh)`, `fillna` (value/dict/ffill/bfill/mean), `duplicated`, `drop_duplicates(subset, keep)`, `replace` | pandas, numpy |
| [airline_schedule_wrangling_assignment](airline_schedule_wrangling_assignment.ipynb) | Airline schedule cleaning and SJU→TUS connection assignment | `skiprows`/`nrows`/`skipfooter`, header merge, `isin`, `~`, `concat`, `merge` | pandas |
| [airline_connections_wrangling](airline_connections_wrangling.ipynb) | Continuation: time parsing and connection filtering | `to_datetime`, set intersection, minimum connection time, `merge(how='cross'/'left'/'right')` | pandas |
| [groupby_pivot_join](groupby_pivot_join.ipynb) | Group-by, pivot and join | `map`, `groupby().sum()`, `pivot`, `join` (index and `on=`), `set_index` | pandas |
| [covid_county_cases](covid_county_cases.ipynb) | U.S. county COVID-19 cases/deaths, 2020 and 2021 | Null checks, `fillna`, `merge`, `join`, `pivot_table`, `groupby` | pandas |
| [suicide_disasters_project_start](suicide_disasters_project_start.ipynb) | Start of a project: CDC suicide data and EM-DAT disasters | `read_csv`, `read_excel`, xlsx to csv conversion | pandas, openpyxl |
| [matplotlib_fundamentals](matplotlib_fundamentals.ipynb) | matplotlib fundamentals | Implicit vs. explicit API, subplots, styles, ticks, labels, legends | matplotlib, numpy |
...
|---|---|---|
| [`suicides.csv`](suicides.csv) | U.S. suicide deaths by state and year, **2001–2020**: deaths, population, crude and age-adjusted rates, years of potential life lost. The footer says it was produced by the CDC National Center for Injury Prevention and Control ([WISQARS](https://wisqars.cdc.gov/)). | suicide_disasters_project_start |
| [`disasters.xlsx`](disasters.xlsx) | [EM-DAT](https://www.emdat.be/) custom export (version 2023-05-08): 1,364 disaster records for the United States and Puerto Rico, 1960–2023 | suicide_disasters_project_start |
| [`Test.csv`](Test.csv) | CSV conversion of `disasters.xlsx`, written by *suicide_disasters_project_start* | suicide_disasters_project_start |
| [`datasets/iris.csv`](datasets/iris.csv) | Iris flower measurements (150 rows, 3 species) | seaborn_plotly_choropleths |
| [`datasets/2012_Election_Data.csv`](datasets/2012_Election_Data.csv) | 2012 U.S. turnout by state: VEP/VAP, ballots, non-citizen %, prison/probation/parole counts | seaborn_plotly_choropleths |

**Referenced but not included:** `ConfirmedFSMarch2023.csv` (airline schedule for March 1–31, 2023; used by *pandas_statistics_and_filtering* and *airline_schedule_wrangling_assignment*, and read from `datasets/ConfirmedFSMarch2023.csv` by *airline_connections_wrangling*), `us-counties-2020.csv` and `us-counties-2021.csv` (*covid_county_cases*), the happiness table shown in *pandas_statistics_and_filtering*'s saved outputs, and `ranDat.csv` (written and then read back by *pandas_series_and_dataframes*).

---

## pandas_series_and_dataframes
...
## pandas_statistics_and_filtering
...
## data_cleaning_missing_values_duplicates
...
## airline_schedule_wrangling_assignment
...
## suicide_disasters_project_start
...
- *pandas_series_and_dataframes* uses `DataFrame.append`, which needs **pandas < 2.0**, or replace it with `pd.concat`.

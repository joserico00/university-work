...
def main():
    ...
    for name, ip, netmask in ipv4_interfaces:
        ...
        nm.scan(hosts=str(network), arguments='-p- -sV -O --script=default -T4 --open -A --host-timeout 10m -v ')
        print("scan done")
        for host in nm.all_hosts():
            print("going through",host)
            print(nm[host])
...
scan_timestamp]
    ...
    columns=['IP Range', 'IP', 'Hostname', 'MAC Address', 'OS Details', 'Protocol', 'Port', 'Name', 'State', 'Product', 'Version', 'Extra Info', 
'Timestamp']

...
def network_discovery():
    ...
    for name, ip, netmask in ipv4_interfaces:
        ...
        if str(network) == "127.0.0.0/8":
            continue  # Skip localhost network

        print(f"Discovering hosts in IP range {network}...")
        nm.scan(hosts=str(network), arguments='-sn -T4')  # Ping scan to find active hosts
    ...
    if active_hosts:
        ...
        print(f"Active hosts saved to active_hosts.csv ({len(active_hosts)} hosts found).")
    else:

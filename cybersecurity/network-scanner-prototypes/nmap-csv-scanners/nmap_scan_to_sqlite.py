...
def main():
    ...
    if not ipv4_interfaces:
        ...
        sys.exit(1)
    conn = sqlite3.connect('networklogs')
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS network (IP, Hostname, MAC Address, OS Details, Protocol, Port, Name, State, Product, Version, Extra Info)')
    nm = nmap.PortScanner()
        ...
        for host in nm.all_hosts():
            ...
            for proto in nm[host].all_protocols():
                for port in nm[host][proto]:
                    ...
                    row = [host, ', '.join(hostnames), mac_address, os_details, proto, port, name, state, product, version, extrainfo]
                    scan_results.append(row)

    df = pd.DataFrame(scan_results, columns=['IP', 'Hostname', 'MAC Address', 'OS Details', 'Protocol', 'Port', 'Name', 'State', 'Product', 'Version', 'Extra Info'])
    df.to_sql('network', conn, if_exists='replace', index = False)

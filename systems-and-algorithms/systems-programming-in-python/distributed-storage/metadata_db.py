# SQLite access layer for the metadata server
...
class MetadataDB:
	...
	def CheckNode(self, address, port):
		"""Check if node is in database and returns name, address, port
                   for connection.
		"""
		#had to change query to add adress and port to make it a tuple
		query = """select nid,address,port from dnode where address="%s" and port=%s""" % (address, port)
		try:
			self.c.execute(query)
		except:
			return None
		return self.c.fetchone() #if it had [0] it only returned the id
	...
	def AddBlockToInode(self, fname, blocks):
		...
		for address, port, chunkid in blocks:
			nid = self.CheckNode(str(address), port)
			if nid:
				query = """insert into block (nid, fid, cid) values (%s, %s, "%s")""" % (str(nid[0]), str(fid), chunkid)
				print "checking the query"
				print nid ,fid , chunkid
				print query
				self.c.execute(query)
			else:
				return 0 
